...
public class PlayerMoveController : MonoBehaviour {
	...
	// Cursor settings
	public float cursorPlaneHeight = 0.15f;
	...
	void  Awake (){		
		motor = (MovementMotor) gameObject.GetComponent<FreeMovementMotor>();
		character = GameObject.Find("Player").transform;
		cursorPrefab = Resources.Load("Cursor", typeof(GameObject)) as GameObject;
...
//	}
	...
	void  Update (){
		...
		#else
		screenMovementSpace = Quaternion.Euler (mainCameraTransform.eulerAngles.x, mainCameraTransform.eulerAngles.y, mainCameraTransform.eulerAngles.z);
		screenMovementForward = screenMovementSpace * Vector3.forward;
		screenMovementRight = screenMovementSpace * Vector3.right;
