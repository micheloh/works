    ...
    <script>
        function inserirNovoTopico()
        {
            ...
            if (nome!=null)
            {
                ...
                if(segmento!=null)
                {
                    window.location = "cpVagasArubaito.php?insere=" + nome + "&segmento=" + segmento;
                }
            }
        }
        ...
        function deletarTopico()
        {
            ...
            {
                window.location = "cpVagasArubaito.php?deleta=" + document.getElementById("cpVagasEmpresasEmpresa").value + "&segmento=" + document.getElementById("cpVagasEmpresasSegmento").value;
            }
        }          

        function mudarSegmento()
        {
            window.location = "cpVagasArubaito.php?segmento=" + document.getElementById("cpVagasEmpresasSegmento").value;
        }
        
        function mudarEmpresa()
        {
            window.location = "cpVagasArubaito.php?verifica=" + document.getElementById("cpVagasEmpresasEmpresa").value + "&segmento=" + document.getElementById("cpVagasEmpresasSegmento").value;
        }        
    ...
    <body>
        <div id="cpConteinerPrincipal">
            ...
            if ($formEmpresa !== NULL && $formDesc !== NULL && $formSegmento !== NULL) {
                ...
                if ($statement = $mysqli->prepare('UPDATE ajTravelEmpresas set empresaDesc = ? where empresaId = ? and empresaTipo = 0')) {
                ...
                if ($formSegmento != NULL) {
                    ...
                    if ($formEmpresaInsert !== NULL) {
                        if ($statement = $mysqli->prepare('INSERT INTO ajTravelEmpresas (empresaNome,segmentoId,empresaTipo) values (?,?,0)')) {
                    ...
                    if ($formEmpresaDelete !== NULL) {
                        if ($statement2 = $mysqli->prepare('DELETE FROM ajTravelEmpresas where empresaId =? and empresaTipo = 0')) {
                    ...
                    if ($formEmpresa == NULL) {
                        if ($statement2 = $mysqli->prepare('SELECT empresaId FROM ajTravelEmpresas where segmentoId = ' . $formSegmento . ' and empresaTipo = 0 ORDER BY empresaId LIMIT 1')) {
                            if ($statement2->execute()) {
                    ...
                    if ($statement2 = $mysqli->prepare('SELECT empresaId,segmentoId FROM ajTravelEmpresas where empresaTipo = 0 ORDER BY segmentoId LIMIT 1')) {
                        if ($statement2->execute()) {
            ...
            <div id="conteudoPagina">
                ...
                <form action="cpVagasArubaito.php" id="VagasEmpresasForm" method="post">
				...
				from ajTravelEmpresas
                    ...
                    if ($formEmpresa !== NULL) {
                        if ($statement = $mysqli->prepare('SELECT empresaDesc FROM ajTravelEmpresas where empresaTipo = 0 and empresaId = ' . $formEmpresa)) {
                            if ($statement->execute()) {
