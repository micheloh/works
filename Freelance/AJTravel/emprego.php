    ...
    <body>
        <div id="conteinerPrincipal">
            ...
            if (!empty($_GET["segmento"])) {
                ...
                $numeroSegmento = 0;
            if ($numeroSegmento == 0) {
                if ($statement = $mysqli->prepare('SELECT segmentoId,segmentoURL,segmentoTitulo FROM ajTravelSegmentos where segmentoId in (select segmentoId from ajTravelEmpresas where empresaTipo = 1) order by segmentoId limit 0,1')) {
                ...
                if ($statement = $mysqli->prepare('SELECT segmentoId,segmentoURL,segmentoTitulo FROM ajTravelSegmentos where segmentoId = ' . $numeroSegmento . ' and segmentoId in (select segmentoId from ajTravelEmpresas where empresaTipo = 1) order by segmentoId limit 0,1')) {
                    if ($statement->execute()) {
                        $statement->bind_result($numeroSegmento, $segmentoURL, $segmentoTitulo);
                        if ($statement->fetch()) {
            ...
            <div id="listaSegmentos">
                ...
                <div id="listagem">
                    <ul>
                        <?php
                        if ($statement = $mysqli->prepare('SELECT segmentoTitulo,segmentoId FROM ajTravelSegmentos where segmentoId in (select segmentoId from ajTravelEmpresas where empresaTipo = 1)')) {
                            if ($statement->execute()) {
                                $statement->bind_result($tituloSegmento, $idSegmento);
                                while ($statement->fetch()) {
                                    ?>
                                    <li>
                                        <a href="/emprego.php?segmento=<?php echo $idSegmento; ?>">
            ...
            <div id="listaConteudo">
                ...
                <div id="listaConteudoInterno">
                    ...
                    if ($statement = $mysqli->prepare('SELECT empresaNome, empresaDesc
                                                                            ...
                                                                            WHERE segmentoId = ' . $numeroSegmento . ' and empresaTipo = 1')) {
                        if ($statement->execute()) {
